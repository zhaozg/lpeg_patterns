...
local function parse_ini()
  local sep = '='
  local SEP      = SP^0 * P(sep) * SP^0
  local pair    = SP^0 * Cg(name * SEP * values) * CRLF
  local nvpairs = Cf(Ct"" * (blank + pair)^0,doset)

  local sname   = SP^0 * P"[" * SP^0 * name * SP^0 * P"]" * comment^-1 * CRLF
  local section = Cg(sname * nvpairs)

  return Cf(Ct"" * (blank + section + pair)^1,doset)
end

local function parse_resolve()
  local SEP      = SP^1
  local pair    = SP^0 * Cg(name * SEP * values) * CRLF

  return Cf(Ct"" * (blank + pair)^1,doset)
end

local M = {}
M.rules = {
  ini = parse_ini(),
  resolve = parse_resolve()
}

-- **********************************************************************

function M.parse(txt, rule)
  rule = rule or 'ini'
  assert(M.rules[rule])
  rule = M.rules[rule]
  return rule:match(txt)
end
